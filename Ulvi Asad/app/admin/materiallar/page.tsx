...
export default function AdminMaterialsPage() {
  ...
  useEffect(() => {
    if (editingMaterial) {
      ...
      // Show existing file as "uploaded"
      setUploaded({
        url:      editingMaterial.fileUrl,
        fileType: editingMaterial.fileType,
        fileName: editingMaterial.fileUrl.split("/").pop() || editingMaterial.title,
        size:     0,
      });
      setShowForm(true);
    }
        ...
        <div className="card-static mb-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-lg font-semibold text-slate-800">
              {editingMaterial ? "Materialı Düzəlt" : "Material Əlavə Et"}
            </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <label className={labelCls}>
                ...
                {editingMaterial && (
                  <span className="ml-2 text-xs font-normal text-slate-400">
                    (yeni fayl seçsəniz köhnəsi əvəz olunacaq)
                  </span>
                )}
              </label>
              <FileUpload
                ...
                onClear={() => {
                  // Edit modunda faylı silsə köhnəni saxla
                  if (editingMaterial) {
                    setUploaded({
                      url:      editingMaterial.fileUrl,
                      fileType: editingMaterial.fileType,
            ...
            <div>
              ...
              <div className="flex gap-2">
                {[
                  ...
                  { value: "STUDENT_ONLY", label: "🔒 Tələbə" },
                ].map((v) => (
            ...
            <div className="flex gap-3 pt-1">
              <button type="submit" disabled={saving || !uploaded}
                className="btn-primary flex items-center gap-2">
                {saving
                  ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                  : (editingMaterial ? "Yadda Saxla" : "Əlavə Et")}
              </button>
                ...
                {materials.map((m) => (
                  <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 pr-4">
                      <span className="badge-category">{getCategoryLabel(m.category)}</span>
                    </td>
