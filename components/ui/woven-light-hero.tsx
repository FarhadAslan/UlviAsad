...
const WovenCanvas = () => {
  ...
  useEffect(() => {
    ...
    // Reusable vectors — GC pressure-i azaltmaq üçün
    ...
    const animate = () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        _cur.set(pos[ix], pos[iy], pos[iz]);
        _org.set(orig[ix], orig[iy], orig[iz]);
        _v.set(vel[ix], vel[iy], vel[iz]);
        ...
        if (d < 1.5 && d > 0) {
          const f = (1.5 - d) * 0.012 / d;
          _v.x += dx * f;
          _v.y += dy * f;
        }

        // Spring back to origin
        _v.x += (_org.x - _cur.x) * 0.001;
        _v.y += (_org.y - _cur.y) * 0.001;
        _v.z += (_org.z - _cur.z) * 0.001;

        // Damping
        _v.multiplyScalar(0.95);

        pos[ix] += _v.x; pos[iy] += _v.y; pos[iz] += _v.z;
        vel[ix]  = _v.x; vel[iy]  = _v.y; vel[iz]  = _v.z;
      }
    ...
    <div className="relative flex min-h-[88vh] w-full flex-col items-center justify-center overflow-hidden"
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0 }} animate={btnCtrl}
          ...
          <Link href="/quizler"
            className="pulsating-btn px-10 py-3.5 text-base font-semibold rounded-[10px] text-white"
