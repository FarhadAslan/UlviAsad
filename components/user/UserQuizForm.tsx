...
export default function UserQuizForm({ quiz, onSuccess, onCancel, preselectedBotId }: UserQuizFormProps) {
  ...
  const handleImageUpload = async (qi: number, file: File) => {
    if (!file.type.startsWith("image/")) {
      error("Yalnız şəkil faylı yükləyə bilərsiniz");
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (invalid) {
      error("Hər sualda ya mətn, ya şəkil, ya da hər ikisi olmalıdır");
      return;
    }
    if (
      questions.some(
        (q) =>
          q.questionType !== "OPEN" && q.options.some((o: any) => !o.text)
      )
    ) {
      error("Bütün cavab seçimlərini doldurun");
      return;
    }
    if (form.type === "SINAQ" && (!form.duration || Number(form.duration) < 1)) {
      error("Sınaq üçün müddət daxil edin (minimum 1 dəqiqə)");
      return;
    }
    setLoading(true);
    try {
          ...
          {questions.map((q, qi) => (
            <div key={qi} className="card-static">
              <div className="flex items-center justify-between mb-3">
                <span className="text-sm font-semibold text-[#1a7fe0]">
                  Sual {qi + 1}
                </span>
                <div className="flex items-center gap-2">
                  <button
                        ...
                        p.map((x, i) =>
                          i === qi
                            ? {
                                ...x,
                                questionType:
                                  x.questionType === "OPEN" ? "CHOICE" : "OPEN",
                              }
                            : x
                        )
                      )
              ...
              <div className="mb-4">
                <label className={labelCls}>
                  Sual şəkli{" "}
                  <span className="text-slate-400 text-xs">(isteğe bağlı)</span>
                </label>
                {q.imageUrl ? (
                  <div className="relative inline-block">
                    ...
                    <img
                      src={q.imageUrl}
                      alt={`Sual ${qi + 1}`}
                      className="rounded-xl border border-slate-200 max-h-48 object-contain"
                    />
                    <button
                      type="button"
                      onClick={() =>
                        setQuestions((p) =>
                          p.map((x, i) => (i === qi ? { ...x, imageUrl: "" } : x))
                        )
                      }
                  ...
                  >
                    {uploadingIdx === qi ? (
                      <>
                        <Loader2 size={22} className="text-[#1a7fe0] animate-spin" />
              ...
              {/* Variantlar — CHOICE tipi */}
              {q.questionType !== "OPEN" && (
                <div className="space-y-2 mb-4">
                  {q.options.map((opt: any, oi: number) => (
                    <div key={opt.label} className="flex items-center gap-3">
                      <span
                        className="w-8 h-8 rounded-lg flex items-center justify-center font-bold text-sm flex-shrink-0 text-[#1a7fe0]"
                        style={{
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setQuestions((p) =>
                          p.map((x, i) =>
                            i === qi ? { ...x, correctOption: e.target.value } : x
                          )
                        )
                      }
                    />
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setQuestions((p) =>
                            p.map((x, i) =>
                              i === qi ? { ...x, points: Math.max(1, val) } : x
                            )
                          );
                        }}
                      />
